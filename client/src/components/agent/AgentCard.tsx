...
const InfoBar = ({icon, name}: InfoBarProps) => (
  <Stack flex={1} minWidth={{ xs: '100%', sm: 300}} gap={1.5} direction="row">{icon}
...
const AgentCard = ({id, name, email, avatar, noOfProperties }: AgentCardProp) => {
  ...
  return (
    <Box component={Link} to={generateLink()} width="100%" sx={{display: 'flex', flexDirection: {xs: "column", sm: "row"}, gap: "20px", padding: "20px", "&hover": { boxShadow: '0 22px 45px 2px rgba(176, 176, 176, 0.1)'}}}>
